...
<div class="layui-layout layui-layout-admin">
		...
		<div class="layui-body layui-fluid">
			...
			<table class="layui-table" id="test" width="600">
				...
				<tr>
					<th width="60">货单编号(仅展示,不可修改)</th>
					<th width="60">货物名</th>
					<th width="60">货物数量</th>
					<th width="60">操作员编号</th>
					<th width="60">类型编号</th>
					<th width="60">生产日期</th>
					<th width="60">货单更新日期(仅展示,不可修改)</th>
					<th width="60">操作</th>
				</tr>

				<c:forEach items="${cargos}" var="cargoVo">
					<form name="cargos" class="layui-form layui-form-pane"
						action="${pageContext.request.contextPath}/Cargo/updateCargo.action">
						<input type="text" name="opId" value="${opId}" hidden="hidden" />
						<tr>
							<td><input type="text" name="orderId" hidden="hidden"
								value="${cargoVo.orderId}" />${cargoVo.orderId}</td>
							<td><input type="text" name="cargoName"
								value="${cargoVo.cargoName}" /></td>
							<td><input type="text" name="cargoAmount"
								value="${cargoVo.cargoAmount}" /></td>
							<td><input type="text" hidden="hidden" name="oNum"
								value="${cargoVo.oNum}" />${cargoVo.oNum}</td>
							<td><input type="text" name="tNum" value="${cargoVo.tNum}" /></td>
							<td><input type="text" name="productDate"
								value="${cargoVo.productDate}" placeholder="日期格式：xxxx-xx-xx" /></td>
							<td><input type="text" hidden="hidden" name="lastOptime"
								value="${cargoVo.lastOptime}" />${cargoVo.lastOptime}</td>
